...
class Genome():
    ...
    def update_hash(self):
        ...
        """
        genh =  self.geneparam['activationL'] + self.geneparam['activationR'] 
    ...
    def print_genome(self):
        ...
        genprint =  self.geneparam['activationL'] + str('_') + str(self.generation) + str('_') + self.geneparam['activationR'] + \
                    str('_') + str(self.accuracy) 

        f = open(str(self.generation-1) + '_Generation_Log.txt', 'a')
        f.write('\n' +  '%s ' % genprint)
        f.close()
