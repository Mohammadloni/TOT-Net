...
class Evolver():
    ...
    def evolve(self, pop):
        ...
        # after_sort = fast_non_dominated_sort_grade(graded)
        graded = []
        for genome in pop:
            graded.append(genome)
        # # print("***************2***************")
        # # for genome in after_sort:
        # for i in range(0, len(after_sort)):
        #     # print(" neruronns: %d "% (after_sort[i].neurons))
        #     # print(" accurracy : %.2f%% "% (after_sort[i].accuracy))
        #     for j in range(0, len(after_sort[i])):
        #         graded.append(after_sort[i][j])
