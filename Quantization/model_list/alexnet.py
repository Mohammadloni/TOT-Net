...
"""class BinConv2d(nn.Module): # change the name of BinConv2d
    def __init__(self, input_channels, output_channels, previous_conv=False,
            kernel_size=-1, stride=-1, padding=-1, groups=1, dropout=0, 
            Linear=False, size1=0,  flag=0):
        ...
        if not self.Linear:
            ...
            if self.previous_conv:
                self.bn = nn.BatchNorm2d(input_channels/size1, eps=1e-4, momentum=0.1, affine=True)
            else:
...
class AlexNet(nn.Module):
    ...
    def __init__(self, num_classes=1000):
        ...
        """self.classifier = nn.Sequential(
            BinConv2d(256 * 6 * 6, 4096, previous_conv=True, Linear=True,size1=36,flag=1),
            BinConv2d(4096, 4096, dropout=0.5, Linear=True,size1=1,flag=0), #previous_conv=True,
            nn.BatchNorm1d(4096, eps=1e-3, momentum=0.1, affine=True),
            nn.Dropout(),
            nn.Linear(4096, num_classes),
        )"""
